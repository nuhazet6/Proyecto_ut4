...
def run(operations_path: Path) -> bool:
    ...
    ERROR_CODES = {"E1":"PRODUCT NOT FOUND","E2":"UNAVAILABLE STOCK","E3":"NOT ENOUGH USER MONEY"}
        ...
        for row in f:
            ...
            match operation_type:
                ...
                case "P":
                    ...
                    price = int(price)
                    error_code = change_price(code, price, machine_status["machine_products"])
                case _:
                    print("Operación no reconocida, lo lamentamos.")
            if error_code:
                error_info = ERROR_CODES[error_code]
                operation_data.append(error_code + " " + error_info)
            operation_result = operation_type + " ".join(operation_data)
            print(f"{' '.join(operation_data)}")
